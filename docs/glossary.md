...
- `stable signal` / `stable-signal image`: the upstream image (`ghcr.io/ublue-os/aurora-dx:latest` by default) used as the cadence signal for scheduled builds. When its digest has not changed since the last promoted image, the scheduled workflow run skips instead of rebuilding. It should always name the same image as `DEFAULT_BASE_IMAGE`, so the gate watches the image the build actually consumes.
...
- `BASE_IMAGE`: base Aurora DX image ref passed to the root `Containerfile`.
